...
def geo_merge(geo1, geo2, adjust=True):
    ...
    dis = [
        Point(coord2[-1]).distance(Point(coord1[0])),
...
def geo_cut(line, geom, adjust=False):
    ...
    for ind, coord in enumerate(coords):
        ...
        if pt_absc == absc:
            ...
            return [
                LineString(coords[: ind + 1]),
                LineString(coords[ind:]),
                Point(coords[ind]),
                0.0,
            ]
        if pt_absc > absc:
            ...
            return [
                LineString(coords[:ind] + [new_c]),
                LineString([new_c] + coords[ind:]),
                Point(new_c),
                dist,
            ]
    return None
...
def nodes_gdf_from_edges_gdf(e_gdf, source=None, target=None, linestring=True):
    ...
    if source in e_gdf.columns:
        ...
        n_gdf = (
            pd.concat([e_gdf["source_geo"], e_gdf["target_geo"]])
            .drop_duplicates()
...
def add_geometry_edges_from_nodes(e_gdf, source, target, n_gdf, node_id):
    ...
    e_gdf = pd.merge(
        e_gdf,
        n_gdf.loc[:, (node_id, GEOM)],
        how="left",
        left_on=source,
        right_on=node_id,
    ).rename(columns={GEOM: "geom_source"})
    e_gdf.pop(node_id)
    e_gdf = pd.merge(
        e_gdf,
        n_gdf.loc[:, (node_id, GEOM)],
        ...
        left_on=target,
        right_on=node_id,
    ).rename(columns={GEOM: "geom_target"})
    e_gdf.pop(node_id)
